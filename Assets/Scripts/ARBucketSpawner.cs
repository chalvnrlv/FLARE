...
public class ARBucketSpawner : MonoBehaviour
{
    [Header("Dependencies")]
    ...
    [SerializeField] private Vector3 spawnRotationEuler = new Vector3(0f, 0f, 0f);
    ...
    [SerializeField] private string mountChildName = "Bucket_mount";
    [SerializeField] private string mountChildFallbackName = "Bucker_mount";
    ...
    private void Update()
    {
        ...
        if (raycastManager == null || bucketPrefab == null)
        {
            return;
        }
    ...
    private bool TrySpawnOnMount()
    {
        ...
        GameObject spawnedObject = Instantiate(bucketPrefab);
    ...
    private Transform ResolveMountTransform()
    {
        ...
        string targetName = !string.IsNullOrEmpty(mountChildName) ? mountChildName : mountChildFallbackName;
        if (string.IsNullOrEmpty(targetName))
        {
            return root.transform;
        }

        GameObject child = SearchHierarchyForName(root, targetName);
        if (child == null && !string.IsNullOrEmpty(mountChildFallbackName))
        {
            child = SearchHierarchyForName(root, mountChildFallbackName);
        }
    ...
    private void DestroyExistingMounted()
    {
        ...
        for (int i = mount.childCount - 1; i >= 0; i--)
        {
            ...
            if (child != null && child.GetComponentInChildren<ARWaterBucket>(true) != null)
            {
    ...
    private void TrySpawnRandomlyAroundPlayer()
    {
        ...
        for (int i = 0; i < maxPlacementAttempts; i++)
        {
            ...
            GameObject spawnedObject = Instantiate(bucketPrefab, spawnPosition, spawnRotation);
            SceneManager.MoveGameObjectToScene(spawnedObject, gameObject.scene);
