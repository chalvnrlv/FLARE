...
public class ARClothSpawner : MonoBehaviour
{
    [Header("Dependencies")]
    ...
    [SerializeField] private string mountChildName = "Lap_mount";
    ...
    private void Update()
    {
        ...
        if (raycastManager == null || clothPrefab == null)
        {
            return;
        }
    ...
    private void DestroyExistingMounted()
    {
        ...
        for (int i = mount.childCount - 1; i >= 0; i--)
        {
            ...
            if (child != null && child.GetComponentInChildren<ARCloth>(true) != null)
            {
    ...
    private void TrySpawnRandomlyAroundPlayer()
    {
        ...
        for (int i = 0; i < maxPlacementAttempts; i++)
        {
            ...
            GameObject spawnedObject = Instantiate(clothPrefab, spawnPosition, spawnRotation);
            SceneManager.MoveGameObjectToScene(spawnedObject, gameObject.scene);
            ...
            hasSpawned = true;
            Debug.Log("[ARClothSpawner] Cloth spawned successfully");
            return;
        }

        Debug.Log("[ARClothSpawner] Could not find suitable placement after " + maxPlacementAttempts + " attempts");
    }
    ...
    private bool IsFarEnoughFromFireSource(Vector3 position)
    {
        if (fireSourcePlacer == null || !fireSourcePlacer.HasPlacedObject)
        {
            return true;
        ...
        GameObject fireSourceObject = fireSourcePlacer.PlacedObject;
        if (fireSourceObject == null)
        {
            return true;
        }

        float distance = Vector3.Distance(position, fireSourceObject.transform.position);
        return distance >= minDistanceFromFireSource;
    }
